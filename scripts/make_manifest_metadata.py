...
def main(input_folder: str, outfp_manifest: str, outfp_metadata: str) -> None:
    ...
    fp_forward = [os.path.realpath(_) for _ in files if "_R1_" in _]
    name_forward = [format_basename(_.replace("_R1_", "")) for _ in files if "_R1_" in _]

    fp_reverse = [os.path.realpath(_) for _ in files if "_R2_" in _]
    name_reverse = [format_basename(_.replace("_R2_", "")) for _ in files if "_R2_" in _]
